...
const Home: NextPage = () => {
  ...
  const stakeTokens = async () => {
    ...
    try {
      await contractFT.approve(contractFTS.address, big,{ from:account, gas:50000});
      
      await contractFTS.stakeTokens(big, {
        from: account,
        gas: 500000 // Gas limit
      });
    } catch (error) {
  ...
  const withdrawTokens = async () => {
    const big = BigInt(inputValue * 10**18);
    try {
      await contractFTS.withdrawTokens(big, {
        from: account,
        gas: 500000 // Gas limit
      });
    } catch (error) {
...
const HeaderFT = () => {
  return (
    <VStack>
       {/* <Head>
        <title>Famous Token </title>
        <meta name="description" content="FamousToken" />
        <link rel="icon" href="/favicon.ico" />
      </Head> */}
      <Box h='5px'/>
      <HStack w='full'>
        <Spacer />
        <Heading size='2xl'>Famous Token </Heading>
        <Spacer />
      </HStack>
      <Box h='10px'/>
    </VStack>
